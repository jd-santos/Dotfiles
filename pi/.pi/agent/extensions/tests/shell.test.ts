...
const { KeybindingsManager } = await import(join(piDir, "dist/core/keybindings.js"));
const { visibleWidth } = await import(tuiPath);

function makeTheme(name = "catppuccin-mocha") {
	const json = JSON.parse(readFileSync(file(`../../themes/${name}.json`), "utf8"));
	const resolve = (value: string) => json.vars[value] ? resolve(json.vars[value]) : value;
	const colors = Object.fromEntries(Object.entries(json.colors).map(([key, value]) => [key, resolve(value as string)]));
...
function harness(order = [footerModule.default, editorModule.default]) {
	...
	const tui = { requestRender() { renders++; }, terminal: { rows: 24 } };
	const data = {
		getGitBranch: () => branch,
		getExtensionStatuses: () => statuses,
		onBranchChange(callback: () => void) { branches.add(callback); return () => branches.delete(callback); },
	};
	const ctx = {
		cwd: "/workspace/projects/Dotfiles",
		hasUI: true,
		model: { id: "openai/gpt-5.6-luna", provider: "openrouter", contextWindow: 128000 },
		modelRegistry: { isUsingOAuth: () => false },
		getContextUsage: () => ({ tokens: 48640, percent: 38, contextWindow: 128000 }),
		sessionManager: { getBranch: () => entries },
		ui: {
			theme,
			setStatus(name: string, text: string) { statuses.set(name, text); },
			setTitle(text: string) { title = text; },
			setFooter(factory: any) { footer?.dispose(); footer = factory?.(tui, theme, data); },
			setEditorComponent(factory: any) {
				editor = factory(tui, { borderColor: (text: string) => theme.fg("border", text), selectList: { selectedPrefix: (text: string) => text, selectedText: (text: string) => text, description: (text: string) => text, scrollInfo: (text: string) => text, noMatch: (text: string) => text } }, new KeybindingsManager());
			},
	...
	return { ctx, theme, statuses, entries, branches, listeners, commands, emit,
		get footer() { return footer; }, get editor() { return editor; }, get title() { return title; },
		get renders() { return renders; },
		changeBranch(value: string) { branch = value; for (const callback of branches) callback(); },
	};
}
...
test("long paths, branches, Unicode, and narrow borders stay within terminal width", () => {
	...
	for (const width of [1, 8, 20, 40, 60, 80, 120]) {
		const data = { getGitBranch: () => "feature/very-long-branch-修正-design", getExtensionStatuses: () => new Map([["permission-gate", "yolo | +3"]]) };
		const label = layout.workspaceLabel(theme, "/a/very/long/parent/directory/项目👩‍💻", data, Math.max(0, width - 4));
		assert.ok(visibleWidth(layout.border(theme, label, width)) <= width);
		if (width >= 40) assert.match(layout.plain(label), /git .*perm yolo/);
	}
	assert.equal(layout.plain(layout.location(theme, "/long/parent/Dotfiles", 13)), "…/Dotfiles");
});

test("context thresholds and unknown capacity match the planning rules", () => {
	const theme = makeTheme();
	for (const [percent, color] of [[49, "syntaxOperator"], [50, "warning"], [79, "warning"], [80, "error"]] as const) {
	...
	for (const percent of [null, undefined, NaN, Infinity]) {
		const text = layout.plain(layout.contextMeter(theme, { percent }, 128000, 60));
		assert.match(text, /unavailable/);
		assert.doesNotMatch(text, /0%|█/);
	}
	assert.match(layout.plain(layout.contextMeter(theme, { tokens: null, percent: 0 }, 128000, 60)), /unavailable/);
});

test("plugin labels are quiet while colored failures survive and sort first", () => {
	const theme = makeTheme();
	const error = theme.fg("error", "LSP Inactive");
	const lines = footerModule.pluginStatuses(new Map([
		["other", "ready"], ["pi-lens-lsp", error], ["usage", "usage: 123 records"],
	]), theme, false);
	...
	const failure = footerModule.pluginStatuses(new Map([
...
test("the permission gate publishes its actual initial and toggled modes", async () => {
	const h = harness([footerModule.default, editorModule.default, gateModule.default]);
...
test("the prompt receives live branch and permission data regardless of load order", async () => {
	for (const order of [[footerModule.default, editorModule.default], [editorModule.default, footerModule.default]]) {
		const h = harness(order);
		await h.emit("session_start");
		assert.match(layout.plain(h.editor.render(100)[0]), /Dotfiles.*git feature\/design.*perm ask/);
		h.changeBranch("main");
		h.statuses.set("permission-gate", "readonly");
		assert.match(layout.plain(h.editor.render(100)[0]), /git main.*perm readonly/);
...
test("autocomplete rows, cursor marker, and scroll indicators survive shell styling", async () => {
	...
	h.editor.autocompleteList = { render: () => ["/model  select model", "/models  scope models"] };
...
test("footer keeps summary emphasis, wraps narrow layouts, and restores details on reload", async () => {
	...
	assert.ok(h.footer.render(80).at(-1).includes(h.theme.bold("A useful session summary")));
	assert.doesNotMatch(h.footer.render(120).map(layout.plain).join("\n"), /cache read/);
...
test("all bundled palettes render the shell with valid color references", () => {
	for (const name of ["catppuccin-mocha", "catppuccin-macchiato", "catppuccin-frappe", "catppuccin-latte", "dracula", "quiet-ink"]) {
