...
`/model` when the provider catalog and auth expose them. The zsh `pi()` wrapper
resolves the OpenRouter `op://` reference through 1Password before launching Pi,
so the key does not live in git.
